...
jQuery(function(){
	...
	if ($.cookie('skin') != null)	{
		...
		jQuery('.lea-all-wrap').attr('data-color-style', $.cookie('skin'));	
		jQuery('.lea-all-wrap').attr('data-gradient', $.cookie('skin'));	
    }
	...
	jQuery('#ts_panel ul.theme.colors li a').click(function(){
		...
		jQuery('.lea-all-wrap').attr('data-color-style', c_skin);				
	  	return false;
    });

	/* Theme Gradient Colors
	-----------------------------------------------*/
	jQuery('#ts_panel ul.theme.gradient li a').click(function(){
		var g_skin = jQuery(this).attr('title');
		$.cookie('skin', g_skin, { expires: 0, path: '/'});		
		jQuery('.slide').attr('data-gradient', g_skin);				
	  	return false;
    });
